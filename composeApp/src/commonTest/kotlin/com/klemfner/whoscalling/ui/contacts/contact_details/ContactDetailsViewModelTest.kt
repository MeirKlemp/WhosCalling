...
class ContactDetailsViewModelTest {
    ...
    fun setup() {
        ...
        viewModel = ContactDetailsViewModel(callLogRepository, spamRepository)
    }
    ...
    fun initialStateIsEmpty() = runTest {
        viewModel.uiState.test {
            val state = awaitItem()
            assertEquals(emptyList(), state.contactCallLogs)
    ...
    fun setSelectedPhoneFiltersCallLogsByNumber() = runTest {
        ...
        viewModel.uiState.test {
            ...
            assertTrue(state.contactCallLogs.all { it.phoneNumber == "+1234567890" })
        }
    }
